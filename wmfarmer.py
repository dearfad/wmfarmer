...
def get_order_info(url_name):  
  order_info = {'name': url_name, 'sell': 0, 'seller': '', 'buy': 0, 'buyer': '',  'status': '', 'time': ''} 
  r = requests.get(f'{items_api_url}/{url_name}/orders', headers={'Platform': 'pc'})  
  if r.status_code == 200:
      order_info['status'] = 'T'
      order_info['time'] = datetime.utcnow().strftime("%H:%M:%S").replace(tzinfo=datetime.timezone.utc).astimezone(tz=None)
...
def item():
  ...
  items = get_items()
  search_result = items[items['item_name_cn'].str.contains(item_name, case=False)]
