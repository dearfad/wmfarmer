...
def get_weapon_price():
    ...
    weapon_price_df = pd.DataFrame(data=np.zeros((len(weapon_prime_list),len(weapon_prime_set_list)+1), dtype = int), index=weapon_prime_list, columns=weapon_prime_set_list.append('item_name'))
